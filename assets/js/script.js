...
var loadPlanner = function () {
  ...
  for (let i = 0; i < 9; i++) {
    ...
    $(
      '<div id="hourBlock' + i + '"class="row col-lg-12 border-top"></div>'
    ).appendTo("#todaySchedule");
    // add the time text + styling(bootstrap)
    $('<h1 class="col-2 p-3">' + schedule.time[i] + "</h1>").appendTo(
      "#hourBlock" + i
    );
    ...
    $(
      '<p id="hourTask' +
        i +
        '" class="col-8 p-3 mb-0">' +
        schedule.hourlyTask[i] +
        "</p>"
    ).appendTo("#hourBlock" + i);
    // add the save button + styling(bootstrap/openIcons)
    $(
      '<button class="col-2 rounded-right" style="font-size: 30px" >+</button>'
    ).appendTo("#hourBlock" + i);
  }
...
$("div").on("click", "p", function () {
  ...
  var changeTaskDescription = $("<textarea>")
    ...
    .val(taskdescription)
    .attr("id", "taskChange");
  changeTaskDescription.attr(
    "placeholder",
    "Add your task here, don't forget to save -->"
  );
...
$("div").on("click", "button", function () {
  ...
  var newTaskDescription = $("#taskChange").val().trim();
